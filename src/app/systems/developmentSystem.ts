...
import { GameStore } from "@/store/rootStore";
import { ResourceType } from "@/types/Resource";
import { TerrainType, Tile } from "@/types/Tile";

// All of these functions are pure and operate on a single tile
...
export const developmentSystem = (state: GameStore): GameStore => {
  const nextTurn = state.turn + 1;

  const newTiles: Tile[][] = state.map.tiles.map(row => row.map(tile => {
    let t: Tile = { ...tile };
