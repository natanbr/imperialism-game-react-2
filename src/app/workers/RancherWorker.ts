...
export function startRancherWork(state: GameState): GameState {
  let updated = false;
  const newTiles = state.map.tiles.map(row =>
    row.map(tile => {
      if (!tile.workers) return tile;
      const worker = tile.workers.find(w => w.type === WorkerType.Rancher && w.status === WorkerStatus.Available);
      ...
      const durationTurns = WorkerLevelDurationsTurns[WorkerType.Rancher]?.[targetLevel as 1 | 2 | 3] || 1;
      const newJob = {
        workerId: worker.id,
      ...
      return {
        ...
        workers: tile.workers.map(w =>
          w.id === worker.id
            ? { ...w, status: WorkerStatus.Working, jobDescription: `Ranching to level ${targetLevel}` }
            : w
        ),
      };
    })
...
export function getRancherActions(tile: Tile, map: GameMap, worker: Worker): PossibleAction | null {
  if (worker.status !== WorkerStatus.Available) return null;
  if (tile.ownerNationId !== worker.nationId) return null;
  if (!tile.developmentJob && RANCHING_TERRAINS.includes(tile.terrain)) {
    const targetLevel = (tile.resource?.level || 0) + 1;
    if (targetLevel > 3) return null;
    return { type: "develop", workerType: WorkerType.Rancher, level: targetLevel as 1 | 2 | 3 };
  }
  return null;
}
