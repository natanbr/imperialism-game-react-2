...
export function startDrillerWork(state: GameState): GameState {
  let updated = false;
  const newTiles = state.map.tiles.map(row =>
    row.map(tile => {
      if (!tile.workers) return tile;
      const worker = tile.workers.find(w => w.type === WorkerType.Driller && w.status === WorkerStatus.Available);
      ...
      const durationTurns = WorkerLevelDurationsTurns[WorkerType.Driller]?.[targetLevel as 1 | 2 | 3] || 1;
      const newJob = {
        workerId: worker.id,
      ...
      return {
        ...
        workers: tile.workers.map(w =>
          w.id === worker.id
            ? { ...w, status: WorkerStatus.Working, jobDescription: `Drilling to level ${targetLevel}` }
            : w
        ),
      };
    })
...
export function getDrillerActions(tile: Tile, map: GameMap, worker: Worker): PossibleAction | null {
  if (worker.status !== WorkerStatus.Available) return null;
  if (tile.ownerNationId !== worker.nationId) return null;
  if (!tile.developmentJob && DRILLING_TERRAINS.includes(tile.terrain)) {
    const targetLevel = (tile.resource?.level || 0) + 1;
    if (targetLevel > 3) return null;
    return { type: "develop", workerType: WorkerType.Driller, level: targetLevel as 1 | 2 | 3 };
  }
  return null;
}
